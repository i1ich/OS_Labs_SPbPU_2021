...
void Tester::CheckWriters(SetType setType, TestType testType, int writersNum, int writingNum) {
  ...
  std::cout << "time = " << (double)time / (double)REPEAT_TIMES << std::endl;
}
...
Set<int> *Tester::CreateSet(Tester::SetType setType) {
  switch (setType) {
...
bool Tester::checkAllItems(Set<int> *set,  std::vector<int> const& array) {
  for (auto item : array) {
...
Tester::ReaderArgs
Tester::CreateReaderArgs(const std::vector<std::vector<int>> &items, int threadIdx, Set<int> *set,
...
void Tester::addToSet(Set<int> *set, const std::vector<int> &array) {
  if (!set)
    return;
  for (auto item : array)
    set->add(item);
